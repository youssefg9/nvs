...
class SRSPilotPattern(PilotPattern):
    ...
    dtype : tf.DType
        Datatype for internal calculations and final output (default: tf.complex64).
    """
    def __init__(self, srs_configs, dtype=tf.complex64):
        ...
        # Allocate uniform mask: [num_tx, max_ports, num_sym, num_sc]
        ...
        for tx in range(num_tx):
            ...
            if srs_grid.ndim != 3:
                raise ValueError(f"SRSConfig {tx} returned srs_grid with shape {srs_grid.shape} (expected 3D).")
            if srs_grid.shape[0] != cfg.num_srs_ports:
                raise ValueError(f"SRSConfig {tx}: srs_grid.shape[0] != num_srs_ports.")
            if srs_grid.shape[1] != num_sc or srs_grid.shape[2] != num_sym:
                raise ValueError(f"SRSConfig {tx}: srs_grid shape mismatch.")
            srs_grid_t = np.transpose(srs_grid, [2, 1, 0])  # shape: [num_sym, num_sc, num_ports]
